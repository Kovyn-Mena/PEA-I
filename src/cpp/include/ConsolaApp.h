...
// 100% Independiente de cualquier GUI para preparación de parcial
// =====================================================================
...
private:
    ...
    // Helpers para lectura segura por teclado
...
public:
    ...
    // Punto de entrada obligatorio (Punto 10 pág 3)
    void iniciar() {
        std::cout << "=================================================================\n";
        ...
        int opcion = leerEntero("Opcion [1 o 2]: ");
        if (opcion == 1) {
    ...
    void menuPrincipal() {
        ...
        while (op != 0) {
            std::cout << "\n=================================================================\n";
            std::cout << "                    MENU PRINCIPAL (C++ Consola)                 \n";
            std::cout << "=================================================================\n";
            ...
            switch (op) {
                ...
                case 0:
                    std::cout << "\n[?] Desea guardar los cambios antes de salir? (1: Si / 2: No): ";
                    if (leerEntero("") == 1) {
                        guardarEnBD();
                    }
                    std::cout << "\n[+] Gracias por utilizar PEA-i UPC. Hasta pronto.\n";
                    break;
                default:
    ...
    void menuGrupos() {
        ...
        while (op != 0) {
            ...
            std::cout << "5. Desactivar Grupo (Borrado Lógico)\n";
            std::cout << "6. Eliminar Grupo Físicamente\n";
            ...
            if (op == 1) {
                ...
                if (!g) std::cout << "  (No hay grupos registrados)\n";
                while (g != nullptr) {
                ...
                if (multi.insertarGrupo(cod, nom, clas, area, lid, anio, true)) {
                    ...
                    std::cout << "[OK] Grupo registrado exitosamente.\n";
                } else {
                    std::cout << "[!] Error: Ya existe un grupo con el código " << cod << "\n";
                }
            } else if (op == 3) {
                std::string cod = leerLinea("Ingrese Código del Grupo: ");
                ...
                if (!g) {
                    ...
                    std::cout << "Estado: " << (g->activo ? "ACTIVO" : "DESACTIVADO") << "\n";
                ...
                if (!g) {
                    ...
                    std::string detallePrevio = "nom=" + g->nombre + ";clas=" + g->clasificacion;
                    ...
                    historial.push("MODIFICAR", "GRUPO", cod, detallePrevio);
                    std::cout << "[OK] Grupo modificado exitosamente.\n";
                }
            } else if (op == 5) {
                std::string cod = leerLinea("Código del Grupo a cambiar estado: ");
                NodoGrupo* g = multi.buscarGrupo(cod);
                if (!g) {
                    std::cout << "[!] Grupo no encontrado.\n";
                } else {
                    ...
                    std::cout << "[OK] Estado cambiado a: " << (nuevoEstado ? "ACTIVO" : "DESACTIVADO") << "\n";
                }
            } else if (op == 6) {
                std::string cod = leerLinea("Código del Grupo a ELIMINAR físicamente: ");
                std::cout << "  [ADVERTENCIA] Esto eliminará el grupo, sus investigadores y productos asociados.\n";
                if (leerEntero("¿Confirmar eliminación física? (1: Sí / 2: Cancelar): ") == 1) {
                    if (multi.eliminarGrupo(cod)) {
                        historial.push("ELIMINAR", "GRUPO", cod, "");
                        std::cout << "[OK] Grupo eliminado físicamente de la memoria.\n";
                    } else {
                        std::cout << "[!] Grupo no encontrado.\n";
                    }
                }
    ...
    void menuInvestigadores() {
        ...
        while (op != 0) {
            ...
            std::cout << "3. Consultar Detalle (Productos del Autor)\n";
            std::cout << "4. Modificar Investigador\n";
            std::cout << "5. Desactivar Investigador (Borrado Lógico)\n";
            ...
            if (op == 1) {
                ...
                if (!multi.buscarGrupo(codG)) {
                    std::cout << "[!] El grupo " << codG << " no existe. Debe crearlo primero.\n";
                    continue;
                }
                ...
                std::string form = leerLinea("Formación Académica (Pregrado, Maestria, Doctorado): ");

                if (multi.insertarInvestigador(codG, doc, nom, cat, form, true)) {
                    historial.push("CREAR", "INVESTIGADOR", doc, codG);
                    std::cout << "[OK] Investigador registrado exitosamente.\n";
                } else {
                    std::cout << "[!] Ya existe un investigador con ese documento.\n";
                }
            } else if (op == 3) {
                std::string doc = leerLinea("Documento del Investigador: ");
                NodoInvestigador* inv = multi.buscarInvestigador(doc);
                if (!inv) {
                    std::cout << "[!] Investigador no encontrado.\n";
                } else {
                    std::cout << "\n==================================================\n";
                ...
                std::string doc = leerLinea("Documento del Investigador a modificar: ");
                NodoInvestigador* inv = multi.buscarInvestigador(doc);
                if (!inv) {
                    std::cout << "[!] Investigador no encontrado.\n";
                } else {
                    std::string detallePrevio = "nom=" + inv->nombre_completo + ";cat=" + inv->categoria;
                    ...
                    historial.push("MODIFICAR", "INVESTIGADOR", doc, detallePrevio);
                    std::cout << "[OK] Investigador modificado exitosamente.\n";
                }
            } else if (op == 5) {
                std::string doc = leerLinea("Documento del Investigador a cambiar estado: ");
                NodoInvestigador* inv = multi.buscarInvestigador(doc);
                if (!inv) {
                    std::cout << "[!] Investigador no encontrado.\n";
                } else {
                    ...
                    std::cout << "[OK] Estado cambiado a: " << (nuevo ? "ACTIVO" : "DESACTIVADO") << "\n";
                }
            } else if (op == 6) {
                std::string doc = leerLinea("Documento del Investigador a ELIMINAR físicamente: ");
                if (leerEntero("¿Confirmar eliminación física? (1: Sí / 2: Cancelar): ") == 1) {
                    if (multi.eliminarInvestigador(doc)) {
                        historial.push("ELIMINAR", "INVESTIGADOR", doc, "");
                        std::cout << "[OK] Investigador y sus productos eliminados de la memoria.\n";
                    } else {
                        std::cout << "[!] Investigador no encontrado.\n";
                    }
                }
    ...
    void menuProductos() {
        ...
        while (op != 0) {
            ...
            std::cout << "5. Desactivar Producto (Borrado Lógico)\n";
            ...
            if (op == 1) {
                ...
                if (!multi.buscarGrupo(codG) || !multi.buscarInvestigador(idInv)) {
                    std::cout << "[!] El Grupo o el Investigador no existen en la Multilista.\n";
                    continue;
                }
                ...
                std::string tit = leerLinea("Título: ");
                int anio = leerEntero("Año: ");
                std::string cat = leerLinea("Categoría MinCiencias (A1, A, B, C): ");
                int val = leerEntero("¿Validado por MinCiencias? (1: Sí / 0: No): ");

                if (multi.insertarProducto(codG, idInv, idP, tipo, tit, anio, cat, val == 1, true)) {
                    historial.push("CREAR", "PRODUCTO", idP, codG);
                    std::cout << "[OK] Producto registrado y enlazado ortogonalmente en memoria.\n";
                } else {
                    std::cout << "[!] Ya existe un producto con el ID " << idP << "\n";
                }
            } else if (op == 3) {
                ...
                if (!p) {
                    ...
                    p->validado = (leerEntero("¿Validado? (1: Sí / 0: No): ") == 1);
                    historial.push("MODIFICAR", "PRODUCTO", idP, prev);
                    std::cout << "[OK] Producto actualizado exitosamente.\n";
                }
            } else if (op == 5) {
                std::string idP = leerLinea("ID del Producto a cambiar estado: ");
                NodoProducto* p = multi.buscarProducto(idP);
                if (!p) {
                    std::cout << "[!] Producto no encontrado.\n";
                } else {
                ...
                std::string idP = leerLinea("ID del Producto a ELIMINAR físicamente: ");
                if (leerEntero("¿Confirmar eliminación física? (1: Sí / 2: Cancelar): ") == 1) {
                    if (multi.eliminarProducto(idP)) {
                        historial.push("ELIMINAR", "PRODUCTO", idP, "");
                        std::cout << "[OK] Producto desenlazado ortogonalmente y liberado de memoria.\n";
                    } else {
                        std::cout << "[!] Producto no encontrado.\n";
                    }
                }
    ...
    void ejecutarDeshacer() {
        ...
        if (acc.tipo_operacion == "DESACTIVAR") {
            // Revertir el estado activo/inactivo
            ...
            std::cout << "[OK] Estado lógico restaurado con éxito.\n";
        } else if (acc.tipo_operacion == "CREAR") {
            // Si se había creado, se retira de la estructura
            ...
            std::cout << "[OK] Inserción revertida (registro eliminado).\n";
        } else {
    ...
    void menuEstadisticasYFiltro() {
        ...
        std::cout << "  [0] Omitir filtro\n";
        int opFiltro = leerEntero("Opción: ");
    ...
    void guardarEnBD() {
        std::cout << "\n[+] Sincronizando Multilista en memoria con la Base de Datos SQLite...\n";
